...
{
    // Reinitialize all middle places as unused
    for(unsigned int i = 1; i < ROWS - 1; ++i)
    {
        for(unsigned int j = 0; j < COLUMS; ++j)
        {
            board_.at(i).at(j) = UNUSED;
        }
    }

    // Reset the places that should be empty
    board_.at(1).at(1) = EMPTY;
    board_.at(2).at(2) = EMPTY;
    board_.at(2).at(1) = EMPTY;
    board_.at(3).at(1) = EMPTY;

    // Reinitialize the top and bottom rows
    board_.at(0) = init_top_or_bottom_row(GREEN);
    board_.at(ROWS - 1) = init_top_or_bottom_row(RED);
}
