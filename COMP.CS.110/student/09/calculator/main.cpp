...
int main() {
    ...
    while ( true ) {
        ...
        if (command_found == 1) {
            for (auto& cmd : COMMANDS) {
                    if (cmd.str == command_to_be_executed) {
                        if (cmd.is_exit && pieces.size() - 1 == cmd.parameter_number){
                            cout << GREETING_AT_END << endl;
                            return 0;
                        } else {
                                ...
                                string_to_double(pieces.at(1), first_parameter);
                                string_to_double(pieces.at(2),second_parameter);
                                double result = cmd.action(first_parameter, second_parameter);
                                cout << result << endl;
                            } else {
